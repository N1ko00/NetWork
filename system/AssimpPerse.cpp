			...
			for (unsigned int t = 0; t < material->GetTextureCount(aiTextureType_DIFFUSE); t++)
			{
				...
				if (AI_SUCCESS == material->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, t), path))
				{
					...
					if (auto tex = pScene->GetEmbeddedTexture(path.C_Str())) {
						...
						// ファイル名だけを取り出す
						std::filesystem::path p = std::filesystem::path(texpath);		// add 2025/12/24 by suzuki

						// ファイル名の UTF-8 を std::string に変換して代入
						texpath = std::string(		
							reinterpret_cast<const char*>(p.filename().u8string().c_str()));	// add 2025/12/24 by suzuki
	
						std::string texname = texturedirectory + "/" + texpath;			// add 2025/12/24 by suzuki
