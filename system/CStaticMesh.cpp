...
bool CStaticMesh::Load(const std::filesystem::path& filepath,const std::filesystem::path& directorypath) {

	std::ifstream ifs(filepath, std::ios::binary);
	if (!ifs) return false;

	const std::string fileUtf8 = utility::PathToUtf8String(filepath);
	const std::string dirUtf8 = utility::PathToUtf8String(directorypath);

	Load(fileUtf8, dirUtf8);
